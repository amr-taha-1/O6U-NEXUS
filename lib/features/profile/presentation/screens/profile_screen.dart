  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return LargeTitleScaffold(
      ...
      body: studentAsync.when(
        data: (student) => _ProfileBody(student: student, unread: unread),
        loading: () => const Padding(
          padding: EdgeInsets.symmetric(horizontal: AppSpacing.screenMargin),
          child: SkeletonListTile(isFirst: true),
        ),
        error: (error, stackTrace) => StatusPlaceholder.error(message: 'Couldn\'t load your profile: $error'),
      ),
    );
  }
}
...
class _ProfileBody extends StatelessWidget {
  const _ProfileBody({required this.student, required this.unread});
